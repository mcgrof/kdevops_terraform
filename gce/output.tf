...
data "null_data_source" "group_hostnames_and_ips" {
  ...
  inputs = {
    ...
    value = "${format("%30s  :  ssh %s@%s %s ", element(google_compute_instance.kdevops_instances.*.name, count.index), var.ssh_username, element(local.ipv4s, count.index), local.ssh_key_i)}"
  }
}
